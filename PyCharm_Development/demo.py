...
def transcribe(filename):
    print("Transcribing...")
    file = f"C:/Users/liamj/PycharmProjects/PyAudio_M_li/{filename}"
    result = subprocess.run(
        [whisper_path, "-m", model_path, "-f", file, "-nt"],
        capture_output=True,
        text=True
    )
...
def gen_ts(x, y):
    ...
    for i in range(len(x_vals)):
        dist = math.sqrt((x_vals[i] - TRUE_SOURCE[0]) ** 2 + (y_vals[i] - TRUE_SOURCE[1]) ** 2)
        t.append(dist / sos)
    t = np.array(t)  # Convert to numpy array

    print(f"TRUE SOURCE: {TRUE_SOURCE}")
    print(f"Generated t values: {[f'{ti:.6f}' for ti in t]}\n")
...
#############################################
try:
    if check_for_word("salt", OUTPUT_FILENAME):
        t = gen_ts(10,10)

        goalx, goaly = run_X_s(t, x_vals, y_vals, z_vals)
        goal = [round(goalx/conversion), round(goaly/conversion)]
